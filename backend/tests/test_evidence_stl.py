"""Unit tests for `app.detection.evidence.stl.detect_stl_residual` -- CLAUDE.md's "every detector
...
def test_raw_evidence_stl_mirrors_the_fired_signals_daily_weekly_path() -> None:
    extra = {(35, h): 20 for h in (1, 2, 3, 4)}
    rows = _seasonal_rows(n_days=40, extra_hours=extra)

    drafts = detect_stl_residual(rows)
    raw = raw_evidence_stl(rows)

    assert len(raw) == len(drafts)
    for r in raw:
        assert r.extractor == EXTRACTOR_STL
        for key in ("observed", "seasonal_expectation", "trend", "residual", "residual_z"):
            assert key in r.measurements
        assert r.contributing_line_numbers
        (query,) = r.baseline_queries
        assert query.historical_prefix == "residual"
        assert query.entity_type in ("user", "src_ip")


def test_raw_evidence_stl_fallback_path_has_null_seasonal_fields() -> None:
    rows = _seasonal_rows(n_days=2, seed=3)
    rows = [r for r in rows if r.ts < _T0 + timedelta(hours=36)]
    spike_hour = _T0 + timedelta(hours=20)
    eid = 100_000
    ...
    assert raw
    for r in raw:
        assert r.measurements["model"] == "fallback_robust_z"
        assert r.measurements["seasonal_expectation"] is None
        assert r.measurements["trend"] is None


def test_raw_evidence_stl_only_covers_hours_that_also_fire_as_signals() -> None:
    rows = _seasonal_rows(n_days=40, seed=2)
    drafts = detect_stl_residual(rows)
    raw = raw_evidence_stl(rows)
    assert len(raw) == len(drafts)
