"""Unit tests for `app.detection.ml.iforest`. Synthetic Gaussian benign population, one fixture
that must fire (a clear multi-feature outlier) and one that must not (an ordinary in-distribution
row) -- CLAUDE.md's own bar for every detector.
"""
...
def test_outlier_scores_higher_than_inlier() -> None:
    ...
    artifact = IsolationForestArtifact.fit(x_train, x_calib)
...
def test_explain_row_shape_and_sign_convention() -> None:
...
def test_save_and_load_round_trip(tmp_path: Path) -> None:
    ...
    artifact.save(path)
    loaded = IsolationForestArtifact.load(path)
