...
explanations, entities, timeline, verdict, plan") — that composite endpoint spans four
milestones' data (`app/graph` signals/entities/timeline, `app/agent` verdict, `app/response`
plan) and is not this module's to build whole. What CLAUDE.md scopes to this file is the verdict
slice specifically:
...
from app.agent.orchestrator import triage_incident, triage_top_incidents_for_analysis
...
def trigger_analysis_triage(
    ...
    verdicts = triage_top_incidents_for_analysis(db, current.tenant.id, analysis_id, force=force)
