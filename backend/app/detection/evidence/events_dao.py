...
class EventRow:
    ...
    # Added for the evidence layer (docs/v2_migration change 2): `EvidencePayload.
    # contributing_line_numbers` wants the *file's* line numbers, not `events.id` (that's what
    # `evidence_event_ids`/`SignalDraft` already carry) -- see `drafts.cap_evidence_rows`.
    # `raw_line_no` defaults to `0` and `bytes_out` to `None` so every pre-existing `EventRow(...)`
    # call site across `tests/test_evidence_*.py` (built before this change) keeps constructing
    # valid rows without passing either.
    raw_line_no: int = 0
